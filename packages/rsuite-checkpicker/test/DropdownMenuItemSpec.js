...
describe('DropdownMenuItem', () => {
  it('Should output a li', () => {
    ...
    assert.ok(instanceDom.querySelector('label').className.match(/\bmenu-item\b/));
    assert.equal(instanceDom.innerText, Title);
  });

  it('Should be active', () => {
    const instance = ReactTestUtils.renderIntoDocument(
      <DropdownMenuItem title="title" active />
    );

    const instanceDom = findDOMNode(instance);
    assert.ok(instanceDom.querySelector('label').className.match(/\bactive\b/));
  });

  it('Should be disabled', () => {
    const instance = ReactTestUtils.renderIntoDocument(
      <DropdownMenuItem title="title" disabled />
    );
  ...
  it('Should call onKeyDown callback', (done) => {
    ...
    const instanceDom = findDOMNode(instance);
    ReactTestUtils.Simulate.keyDown(instanceDom.querySelector('label'));
  });
